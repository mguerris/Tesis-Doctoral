...
################################################################################
  ...
  save(links_SNA, file="links_SNA_AH.rda")
  ...
  save(nodes, file="nodes_SNA_AH.rda")

  topMentioned <- nodes[order(-nodes$inI),]
  rownames(topMentioned) <- (1:nrow(topMentioned))
  save(topMentioned, file="topMentioned_AH.rda")
  topScreenNames <- nodes[order(-nodes$outI),]
  rownames(topScreenNames) <- (1:nrow(topScreenNames))
  save(topScreenNames, file="topScreenNames_AH.rda")
  
  
  #Calcul paràmetres de la xarxa global
  load("links_SNA_AH.rda")
  load("nodes_SNA_AH.rda")
  load("topMentioned_AH.rda")
  load("topScreenNames_AH.rda")
  load("AH_mentions.RData")
  colnames(links_SNA)<-c("Source","Target","Weight")
  write.csv(links_SNA, file="Edge_list_AH.csv", row.names = FALSE)
  ...
  write.csv(distance_matrix,file="Distancia_AH_10nodos.csv")
  
  library("openxlsx")
  write.xlsx(read.csv("Distancia_AH_10nodos.csv"), "Distancia_AH_10nodos.xlsx")
  
  nodes_to_view<-df[1:50,"name"]
  pos_in_net<-which(V(g)$name %in% nodes_to_view)
  distance_matrix<-igraph::distances(g,v=pos_in_net,to=pos_in_net, weights=NA)
  
  write.csv(distance_matrix,file="Distancia_AH_50nodos.csv", row.names = FALSE)
  ...
  write.csv(names_dist1_unique,file="Usuarios_dist1_AH_50nodos.csv")
  write.csv(names_dist1,file="Pares_usuarios_dist1_AH_50nodos.csv")
  library("openxlsx")
  write.xlsx(read.csv("Usuarios_dist1_AH_50nodos.csv"), "Usuarios_dist1_AH_50nodos.xlsx")
  write.xlsx(read.csv("Pares_usuarios_dist1_AH_50nodos.csv"), "Pares_usuarios_dist1_AH_50nodos.xlsx")
  
  
  nodes_to_view<-df[1:500,"name"]
  pos_in_net<-which(V(g)$name %in% nodes_to_view)
  distance_matrix<-igraph::distances(g,v=pos_in_net,to=pos_in_net, weights=NA)
  
  write.csv(distance_matrix,file="Distancia_AH_500nodos.csv", row.names = FALSE)
  ...
  lapply(seq(1:5), function(x) extrae_tweets_unicos(tweets_unique_raw[[x]], mentioned[[x]],"AH"))
