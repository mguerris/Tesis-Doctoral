...
fichero<-"Dtm_f.RData"
load(fichero)
silh<-lapply(best_skm, function(x) silhouette(x[[1]]))
resum_silh<-lapply(silh, function(x) summary(x))

nclus<-clusters
sel_res_kmeans<-resum_silh[!unlist(lapply(resum_silh,is.null))]
crit<-unlist(lapply(sel_res_kmeans ,
                    function(x) x$avg.width))
cls<-clusters
windows(1024,680)
plot(x=cls[1:length(crit)],y=crit)

setwd(".")
save(resum_silh, file="Resum_silouette.RData")
